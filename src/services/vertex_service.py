...
def procesar_guia_ia_vertex(pdf_bytes):
   ...
   # ====================================================================
    ...
    ESTRUCTURA JSON EXACTA Y REGLAS DE NEGOCIO OBLIGATORIAS:
    {
        ...
        "punto_partida": "REGLA DE ORO DE EXTRACCIÓN: Primero, EXAMINA OBLIGATORIAMENTE el bloque 'OBSERVACIONES' de la guía. Busca palabras clave: 'PLANTA EMPACADORA', 'LOS LAURELES', 'LA ESPERANZA' u otros Fundos. Concatena la dirección de partida base con el nombre extraído usando un guion. Ejemplo de Salida Obligatoria: 'Direccion de Partida - PLANTA EMPACADORA' o 'Av. Sur Km 280 - FUNDO LOS LAURELES'. Si el campo está vacío, devuelve únicamente la dirección base de partida sin añadir fundos inventados. IMPORTANTE: Extrae EXACTAMENTE el texto impreso en el documento. BAJO NINGUNA CIRCUNSTANCIA deduzcas, inventes, o autocompletes direcciones basándote en el nombre de la empresa. Si el documento solo dice 'Fundo Pisco', tu respuesta debe ser únicamente 'Fundo Pisco'.", 
